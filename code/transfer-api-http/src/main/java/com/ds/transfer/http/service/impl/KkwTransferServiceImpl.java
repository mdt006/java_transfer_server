...
/**
 *@ClassName: KkwTransferServiceImpl
 * @Description: TODO(KKW业务)
 * @author leo
 * @date 2018年7月11日
 */
...
public class KkwTransferServiceImpl extends CommonTransferService implements KkwTransferService<KkwApiUserEntity> {
	...
	@Resource(name = "kkwTransfer")
	private TransferService<KkwApiUserEntity> kkwTransfer;
	...
	public String transfer(TransferParam param) {
		...
		try {
			...
			remarks = RemarkConstants.KKW_TRANSFER_REMARK.split(",");
			...
			if (IN.equals(type)) {
				...
				param.setLiveId(SysConstants.LiveId.KKW);
				...
				if (SUCCESS.equals(resultMap.get(STATUS))) {
					...
					result = this.kkwTransfer.transfer(param);
					providerIp = printProviderIp();
					logger.info("kkw transfer ip = {}, result = {}", providerIp, result);
				}
			} else {
				if (param.isTotalBalance()) {
			...
			logger.info("KKW transfer username={},siteId={},type={},error:{}!",param.getUsername(),
					param.getEntity().getSiteId(),(type.equals(IN)==true ? product+" -> KKW error!":"KKW -> "+product+" error!"),e);
			e.printStackTrace();
			return JSONUtils.map2Json(maybe("kkw转账异常"));
		}
	...
	public String queryBalance(QueryBalanceParam param) {
		...
		try {
			...
			logger.info("kkw queryBalance-->checkAndCreateMember before username={},siteId={}",param.getUsername(),siteId);
			long startCuTime = System.currentTimeMillis();
			createMemberResultMap = this.kkwTransfer.checkAndCreateMember(userParam);
			long endCuTime = System.currentTimeMillis();
			logger.info("kkw queryBalance-->checkAndCreateMember after username={},siteId={}，elapsedTime={} ms,result={}",param.getUsername(),siteId,(endCuTime-startCuTime),createMemberResultMap.toString());
			if (!SUCCESS.equals(createMemberResultMap.get(STATUS))) {
				return JSONUtils.map2Json(createMemberResultMap);
			}
			logger.info("kkw queryBalance before username={},siteId={},entity ip = {}",param.getUsername(),siteId,param.getIp());
			long startTime = System.currentTimeMillis();
			result = this.kkwTransfer.queryBalance(param);
			long endTime = System.currentTimeMillis();
			providerIp = printProviderIp();
			logger.info("kkw queryBalance after username={},siteId={},ip={},elapsedTime={} ms,result={}",param.getUsername(),siteId,providerIp,(endTime-startTime),result);
			return result;
		} catch (Exception e) {
			logger.error("kkw queryBalance username={},siteId={},ip={},error:{}!",param.getUsername(),siteId,providerIp,e);
			e.printStackTrace();
			return JSONUtils.map2Json(maybe("KKW查询余额异常"));
		}
	}

	public String login(LoginParam param) {
		logger.info("kkw login before username={},siteId={}",param.getUsername(),param.getEntity().getSiteId());
		long startTime = System.currentTimeMillis();
		String result = this.kkwTransfer.login(param);
		long endTime = System.currentTimeMillis();
		String providerIp = printProviderIp();
		logger.info("kkw login after username={},siteId={},ip={},elapsedTime={},result={}", providerIp,(endTime-startTime),result);
		return result;
	}

	@Override
	public String totalBalanceBySiteId(TotalBalanceParam param) {
		logger.info("kkw totalBalanceBySiteId siteId={},liveType={},fromDate={},toDate={}",param.getSiteId(), param.getLiveType(), param.getFromDate(), param.getToDate());
		Double balance = 0.00;
		try {
			String agent = this.transferRecordDao.totalBalanceByAgent(param);
			if (!StringsUtil.isNull(agent)) {
				logger.info("kkw queryTotal agent={}",agent);
				long startTime = System.currentTimeMillis();
				Map<String, Double> resultMap = this.queryBalanceMultipartByAgent(param, agent);
				long endTime = System.currentTimeMillis();
				balance = resultMap.get("balance");
				logger.info("kkw queryTotal agent={},totalBalance={},elapsedTime={} ms",agent,balance,(endTime-startTime));
			}
		} catch (Exception e) {
			logger.info("kkw totalBalanceBySiteId siteId={},fromDate={},toDate={},error:{}!",param.getSiteId(),param.getFromDate(), param.getToDate(),e);
			e.printStackTrace();
	...
	private Map<String, Double> queryBalanceMultipartByAgent(TotalBalanceParam param, String agent) {
		...
		try {
			...
			Map<String, Object> resultData = JSONUtils.json2Map(kkwTransfer.queryAgentBalance(queryBalanceParam));
			if ("10000".equals(resultData.get("status"))) { //成功
				resultMap.put("balance", Double.valueOf((String) resultData.get("balance")));
				return resultMap;
			} else { //失败返回0
				logger.error("kkw agentTotalBalance siteId={},agent={} error!",agent,param.getSiteId());
				resultMap.put("balance",balance);
				return resultMap;
			}
		} catch (Exception e) {
			logger.error("kkw agentTotalBalance siteId={},agent={} error:{}!",agent,param.getSiteId(),e);
			e.printStackTrace();
			resultMap.put("balance",balance);
			return resultMap;
		}
	...
	@Override
	public KkwApiUserEntity queryUserExist(String username) {
		return this.kkwTransfer.queryUserExist(username);
	}

	@Override
	public String queryStatusByBillno(QueryOrderStatusParam param) {
		return this.kkwTransfer.queryStatusByBillno(param);
	}
