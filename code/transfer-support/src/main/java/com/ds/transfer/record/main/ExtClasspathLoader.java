package com.ds.transfer.record.main;
...
//		}
	...
	private static List<String> getJarFiles() {
		...
		if (!file.isDirectory()) {
			throw new RuntimeException("jar文件目录错误");
		}
		File[] listFiles = file.listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
				return pathname.getName().endsWith(".jar") ? true : false;
			}
		});
		for (File f : listFiles) {
			list.add(file.getAbsolutePath() + f.getName());
		}
		return list;
	}
	...
	private static List<String> getResFiles() {
		...
		if (!file.isDirectory()) {
			throw new RuntimeException("资源文件目录错误");
		}
		System.out.println(file.getAbsolutePath());
		File[] listFiles = file.listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
				return (pathname.getName().endsWith(".xml") || pathname.getName().endsWith(".properties")) ? true : false;
			}
		});
		for (File f : listFiles) {
			list.add(file.getAbsolutePath() + f.getName());
